...
@section('title', __('labels.backend.access.accounts.management') . ' | ' . __('labels.backend.access.accounts.edit'))
...
@section('content')
{{ html()->modelForm($account, 'PATCH', route('admin.account.update', $account->id))->class('form-horizontal needs-validation')->open() }}
    <div class="card">
        <div class="card-body">
            <div class="row">
                <div class="col-sm-5">
                    <h4 class="card-title mb-0">
                        ...
                        <small class="text-muted">@lang('labels.backend.access.accounts.edit')</small>
                    </h4>
                </div><!--col-->
            </div><!--row-->
            ...
            <div class="row mt-4 mb-4">
                <div class="col">
                    ...
                    {{ html()->label(__('validation.attributes.backend.access.accounts.ZohoID'))->class('col-md-2 form-control-label')->for('ZohoID') }}

                        <div class="col-sm-10 col-md-6">
                            {{ html()->text('ZohoID')
                                ->class('form-control')
                                ->placeholder(__('validation.attributes.backend.access.accounts.ZohoID'))
                    ...
                    <div class="form-group row">
                        {{ html()->label(__('validation.attributes.backend.access.accounts.map_location'))->class('col-md-2 form-control-label')->for('map_location') }}
                        {{ html()->hidden('ZohoID') }}
                        {{ html()->hidden('lat') }}
                        {{ html()->hidden('lng') }}
                        {{ html()->hidden('formatted_address') }}

                        <div class="col-md-8">
                            <img src="{{static_map($account)}}" class="img-fluid img-thumbnail" height="200" width="400" title="Current location of Account" />
                        </div><!--col-->
                    </div><!--form-group-->
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-md-10">
                            <label class="switch switch-label switch-pill switch-primary">
                                {{ html()->checkbox('override', false)->class('switch-input')->checked($account->override == 1) }}
                                <span class="switch-slider" data-checked="yes" data-unchecked="no"></span>
                            </label>
                            <div class="help-block with-errors">
                            </div>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-10 col-md-6">
                            {{ html()->select('tags[]')
                                ->id('tagSelect')
                                ->class('form-control')
                                ->options(getTagsOptions($account))
...
@endphp
                    <div class="form-group row">
                        {{ html()->label(__('validation.attributes.backend.access.accounts.associated_tags'))
                            ...
                            @if($allTags->count())
                                @foreach($allTags as $tag)
                                    <div class="checkbox d-flex align-items-center">
                                        {{ html()->label(
                                                html()->checkbox('tags[]', hasTag($account->tags, $tag->name), $tag->name)
                                                        ->class('switch-input')
                                                        ->id('tag-'.$tag->slug)
                                                    . '<span class="switch-slider" data-checked="on" data-unchecked="off"></span>')
                                                ->class('switch switch-label switch-pill switch-primary mr-2')
                                            ->for('tag-'.$tag->slug) }}
                                        {{ html()->label(ucwords($tag->name))->for('tag-'.$tag->slug) }}
                                    </div>
                                @endforeach
                            @endif
                        </div><!--col-->
